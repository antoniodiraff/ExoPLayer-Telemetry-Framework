...
public final class EventElementBuilder {
    ...
    public static void ApplicationStartupInfo(String device_vendor, String device_model, String device_so, String codice_cliente) {
        asi = new ASI();
        asi.setStart_time(Listener.getCurrentTimeStamp());
    ...
    */
    ...
    public static void sessionClose(String currentTimeStamp) {
        ...
        sc.setClosing_time(Listener.getCurrentTimeStamp());
        sc.update();
    ...
    public static void addToEventList(PayLoadElement payLoadElement, String name) {
        if (Listener.event != null) {
            Listener.event.events_list.add(new EventElement(name, payLoadElement.getPayload()));
        } else {
    ...
    public static void addToEventList(PayLoadElement payLoadElement, String name, int i) {
        if (Listener.event != null) {
            if(Listener.event.events_list.size() == 0 || Listener.event.events_list == null) {

            }else {

                EventElement eventElement = Listener.event.events_list.get(0);
                if (eventElement.getName() == "ASI") {
                    Listener.event.events_list.add(1, new EventElement(name, payLoadElement.getPayload()));
                } else {
                    Listener.event.events_list.add(0, new EventElement(name, payLoadElement.getPayload()));
                }
            }
        } else {
            Log.i(TAG, "*********    Null Event Object");
