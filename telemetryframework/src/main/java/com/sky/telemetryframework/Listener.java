...
import com.sky.telemetryframework.Model.Event;
import com.sky.telemetryframework.Model.EventElement;
import com.sky.telemetryframework.Model.SSCH;
import com.sky.telemetryframework.Model.SSDVOD;
import com.sky.telemetryframework.Model.SSPVOD;
import com.sky.telemetryframework.Model.SSRCH;
import com.sky.telemetryframework.Model.SSVOD;
import com.sky.telemetryframework.Model.STRB;
import com.sky.telemetryframework.Model.Start;
...
import static com.sky.telemetryframework.EventElementBuilder.sessionPlaybackPause;
import static com.sky.telemetryframework.EventElementBuilder.sessionsTreamingPause;
import static com.sky.telemetryframework.EventElementBuilder.sessionsTreamingReBuferring;
import static com.sky.telemetryframework.EventElementBuilder.sessionsTreamingRestart;


public final class Listener implements Player.EventListener, AudioRendererEventListener, VideoRendererEventListener,
    ...
    private boolean isOnRenderedFirstFrame = false;
    ...
    private boolean isTheFirstTime_startEvent = true;
    private boolean isFirstTime_OnLoadedCompleted = true;
    private boolean isTheFirstTime_Buffering = true;
    private boolean isTheFirstTime_Ready = true;
    int i;
    private StreamingType streamingType;
//endregion

    public Listener(
            //final MappingTrackSelector trackSelector, boolean enableTrace,
            String serverURL, int serverTimeout,
            //String userAgent,
            //String dev_id, String user_extid,
            String source, int dequeueingIntervalTime, Context c, StreamingType streamingType,
            //String originalSessionId,
            String restartSec, boolean isFree,
    ...
    private void ready_CallBack(String currentTimeStamp, boolean play, StreamingType streamingType) {
        ...
        if (isTheFirstTime_Ready) {
            ...
            EventElementBuilder.updateStartEvent(startEvent, name, currentTimeStamp, i);
            isTheFirstTime_Ready = false;
        } else {
          //  if (!isOnRenderedFirstFrame) {
                if (play) {
                    if (isLive || isRestart || isVod) {
    ...
    private void onDownstreamFormatChanged_callback(Event event, String currentTimeStamp, Format trackFormat) {
        ...
        if (event != null && (isLive || isVod || isRestart)) {
            EventElementBuilder.sessionsTreamingChangeLevel(trackFormat, oldBitRate);
        }
        oldBitRate = String.valueOf(trackFormat.bitrate);
    }

    private void onLoadCompletedCallback(DataSpec dataSpec, Format trackFormat, long loadDurationMs, long bytesLoaded) {
        if (trackFormat != null) {
        ...
        if (isFirstTime_OnLoadedCompleted) {
            ...
            if (event != null) {
                ...
                SessionDownload.sessionsTreamingDownload(chunkUri, chunkIndex, chunkType, bitrate, "", "200",
    ...
    public void onStartSession(boolean isOnRestart) {
        if (!isOnRestart) {
            ...
            //listener.createNewEvent(getApplicationContext());
            becomeActive();
        }
    }
    ...
    static void createNewEvent(Context c) {
        ...
        event = new Event(deviceInfo, eventList);
    }
    ...
    public void becomeActive() {
        ...
        if (isLive || isVod || isRestart) {
            ...
            EventElementBuilder.sessionPlaybackRestart(getCurrentTimeStamp());
        }
        isTheFirstTime_Ready = false;
        isTheFirstTime_Buffering = false;
    ...
    private void createStartEvent() {
        ...
        if (isTheFirstTime_startEvent) {
            ...
            switch (streamingType) {
                case Live:
                    ssch = EventElementBuilder.sessionStartCHannel(c, getCurrentTimeStamp(), channelID, channelName, channelEPG, channelType, ipServer);
                    break;
                case Restart:
                    ssrch = EventElementBuilder.sessionStartRestartCHannel(c, getCurrentTimeStamp(), channelID, channelName, channelEPG, channelType, ipServer, restartSec);
                    break;
                case LocalFile:
                    sspvod = EventElementBuilder.sessionStartPlaybackVOD(c, getCurrentTimeStamp(), originalSessionId, offerID, assetTitle, assetSource);
                    break;
                case Vod:
                    ssvod = EventElementBuilder.sessionStartVOD(c, getCurrentTimeStamp(), offerID, assetTitle, assetType, assetSource, ipServer);
                    break;
           //     case Download:
             //       ssdvod = SessionDownload.sessionStartDownloadVod(c, getCurrentTimeStamp(), offerID, assetTitle, assetSource, ipServer);
                default:
                    break;
            }
        }
    ...
    public static void stampaJSON(String jsonInString) {
    ...
    public static String fromObjectToJSON(Object object) throws IOException {
        String jsonInString = null;
       // Gson gson = new Gson();
        //gson.toJson(event);
        ObjectMapper mapper = new ObjectMapper();
    ...
    public void terminate(String currentTimeStamp) {
        if (event != null) {
            if (isLocalFile) {
                EventElementBuilder.sessionPlaybackClose(currentTimeStamp, player);
            } else {
                EventElementBuilder.sessionClose(currentTimeStamp);
            }
        }
    }
    ...
    public void onPlayerError(ExoPlaybackException e) {
        ...
        printInternalError("Player Error", e);
    }

    //region onTracksChanged
  /*  @Override
...
//        Log.d(TAG, " bytesLoaded " + bytesLoaded);
    ...
    public void onLoadCompleted(DataSpec dataSpec, int dataType, int trackType, Format trackFormat,
        ...
        onLoadCompletedCallback(dataSpec, trackFormat, loadDurationMs, bytesLoaded);
//region Log
/*        Log.d(TAG, "dataSpec " + dataSpec);
        Log.d(TAG, " dataType" + dataType);
        Log.d(TAG, " trackType " + trackType);
...
*/
    ...
    public void printInternalError(String type, Exception err) {
